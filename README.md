...
Install PostGIS during setup or manually:"C:\Program Files\PostgreSQL\<version>\bin\psql.exe" -U postgres
...
Create a .env file in backend/ based on .env.example:PORT=5000
DB_HOST=localhost
DB_USER=postgres
...
For a fresh database (warning: drops existing tables), temporarily set:sequelize.sync({ force: true });

Run the backend, then revert to force: false.
Verify tables:psql -h localhost -U postgres -d event_management
\dt
...
Register at /register, log in at /login, and manage zones at /zones.
...
Ensure commits exist and the branch is master:git branch
git add .
...
Clone and Branch:git checkout master
...
Push and Create PR:git push origin feature/<feature-name>
